...
export default function BillingPage() {
  ...
  const load = useCallback(() => {
    setErr(false);
    api.get<Billing>('/admin/billing').then(setData).catch(() => setErr(true));
  }, []);
  useEffect(() => { load(); }, [load]);

  async function choose(plan: Plan) {
    setBusy(plan);
    try { await api.put('/admin/billing/plan', { plan }); push(`${t('bill.switched')} ${plan}.`, 'success'); load(); }
    catch (e: any) { push(e?.message || t('bill.failed'), 'error'); }
  ...
  const lim = (n: number | null) => (n == null ? t('bill.unlimited') : n);

  return (
    <div>
      <div className="page-head"><h1>{t('bill.title')}</h1></div>
      <div className="info-box">{t('bill.info')}</div>

      <div className="grid grid-2" style={{ marginBottom: 22 }}>
        <div className="card">
          <div className="card-sub">{t('bill.current_plan')}</div>
          <div style={{ fontFamily: 'Space Grotesk', fontSize: 26, fontWeight: 700, margin: '2px 0 14px' }}>{data.plan}</div>
          <div style={{ fontSize: 13, fontWeight: 600 }}>{t('bill.users')} <span className="card-sub">· {data.usage.users} / {lim(data.limits.maxUsers)}</span></div>
          <Bar used={data.usage.users} max={data.limits.maxUsers} />
          <div style={{ fontSize: 13, fontWeight: 600, marginTop: 12 }}>{t('bill.resources')} <span className="card-sub">· {data.usage.resources} / {lim(data.limits.maxResources)}</span></div>
          <Bar used={data.usage.resources} max={data.limits.maxResources} />
        </div>
        <div className="card">
          <div className="card-sub">{t('bill.features')}</div>
      ...
      <div className="section-head"><h3>{t('bill.choose')}</h3></div>
          ...
          return (
            <div key={p} className="card" style={{ borderColor: current ? 'var(--teal)' : undefined, borderWidth: current ? 2 : 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {current ? <span className="badge green">{t('bill.current')}</span> : null}
              </div>
              <ul style={{ listStyle: 'none', fontSize: 13, margin: '12px 0', display: 'flex', flexDirection: 'column', gap: 6 }}>
                <li>👥 {l.maxUsers == null ? t('bill.unlimited') : `${t('bill.up_to')} ${l.maxUsers}`} {t('bill.users_unit')}</li>
                <li>🏢 {l.maxResources == null ? t('bill.unlimited') : `${t('bill.up_to')} ${l.maxResources}`} {t('bill.resources_unit')}</li>
                <li>✨ {l.features.length} {t('bill.integrations_unit')}</li>
              </ul>
              <button className={`btn ${current ? 'btn-ghost' : 'btn-primary'}`} style={{ width: '100%' }} disabled={current || busy === p} onClick={() => choose(p)}>
                {current ? t('bill.active') : busy === p ? <span className="spinner" /> : `${t('bill.switch_to')} ${p}`}
              </button>
            </div>
          );
        })}
