...
/**
 * "Forgot password" (tester feedback #3) — mirrors /activate's two-mode shape:
 * without a token this asks for an email and sends the link; with a token it
 * sets a new password and signs the person straight in. Unlike activation,
 * this works on accounts that already have a real password.
 */
  ...
  async function setPasswordSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await api.post<{ accessToken: string; refreshToken: string }>(
        '/auth/password/reset', { token, newPassword: password }, false,
      );
      tokenStore.set(res.accessToken, res.refreshToken);
      // Full navigation so the auth provider re-reads the new session.
      window.location.href = '/dashboard';
    } catch (e: any) {
      setErr(e?.message || t('pwreset.fail'));
      setBusy(false);
    }
  }
  ...
  return (
    <div className="login-wrap">
      <div className="login-hero">
        ...
        <h1>{t('pwreset.hero_title')}</h1>
        <p>{t('pwreset.hero_sub')}</p>
      </div>
      <div className="login-panel">
        {token ? (
          <form className="login-card" onSubmit={setPasswordSubmit}>
            <h2>{t('pwreset.set_title')}</h2>
            <p className="login-sub">{t('pwreset.set_sub')}</p>
            ...
            <button className="btn btn-primary" style={{ width: '100%' }} disabled={busy || !canSet}>
              {busy ? <span className="spinner" /> : t('pwreset.set_submit')}
            </button>
          </form>
        ) : sent ? (
          <div className="login-card">
            <h2>{t('pwreset.sent_title')}</h2>
            <p className="login-sub">{t('pwreset.sent_sub')}</p>
          ...
          <form className="login-card" onSubmit={requestLink}>
            <h2>{t('pwreset.title')}</h2>
            <p className="login-sub">{t('pwreset.sub')}</p>
            <div className="f-group">
            ...
            <button className="btn btn-primary" style={{ width: '100%' }} disabled={busy}>
              {busy ? <span className="spinner" /> : t('pwreset.send')}
            </button>
