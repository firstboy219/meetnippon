 ...
 * older is a coincidence (e.g. a client-side validation toast with no
 * request behind it at all) and would just mislabel an unrelated failure.
 */
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const push = useCallback((text: string, kind: Kind = 'info') => {
    ...
    // Errors stay up longer — there is now something worth reading and a
    // button worth noticing before it vanishes.
  ...
  const reportError = useCallback(async (id: number, detail?: ErrorDetail) => {
    ...
    try {
      await api.post('/error-reports', { app: 'web-user', ...detail });
